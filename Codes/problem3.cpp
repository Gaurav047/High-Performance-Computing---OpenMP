// Compile: g++ -fopenmp problem3.cpp -o problem3
// Run: ./problem3
...
int main() {
  ...
  start_time = omp_get_wtime();
  uint64_t par_sum = par_sum_omp_nored(x); // function 
  end_time = omp_get_wtime();
  ...
  start_time = omp_get_wtime();
  uint64_t ws_sum = par_sum_omp_red(x); // function 
  end_time = omp_get_wtime();
  ...
  start_time = omp_get_wtime();
  uint64_t task_sum = par_sum_omp_tasks(x); // function
  end_time = omp_get_wtime();
  if (seq_sum != task_sum) {
